...
// ALL official answers for ALL years (111-114), keyed by paper code
// Source: official PDF answer key images, manually read
const official = {
  111: {
...
async function run() {
  let totalFixed = 0, totalChecked = 0, totalCorrect = 0;
  ...
  for (const year of [111, 112, 113, 114]) {
    ...
    for (const doc of snap.docs) {
      ...
      const paper = parts[1]; // '2301' or '301'
      const qnum = data.questionNumber;
      
      if (!official[year] || !official[year][paper] || !official[year][paper][qnum]) continue;
      
      totalChecked++;
      const correct = official[year][paper][qnum];
      
      if (data.answer !== correct) {
        await doc.ref.set({ answer: correct }, { merge: true });
        console.log(`FIXED ${id}: ${data.answer} -> ${correct}`);
        yearFixed++;
        totalFixed++;
      } else {
        totalCorrect++;
    ...
    console.log(`--- ${year}: checked ${snap.size}, fixed ${yearFixed} ---`);
  }

  console.log(`\n=== FINAL: ${totalChecked} checked, ${totalCorrect} already correct, ${totalFixed} fixed ===`);
  process.exit();
}
run();
