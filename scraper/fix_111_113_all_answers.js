...
// Official answers (111-113) - read from PDF images
const official = {
  111: {
...
async function run() {
  let totalFixed = 0, totalChecked = 0, totalMissing = 0;

  for (const year of [111, 112, 113]) {
    const data = JSON.parse(fs.readFileSync(`data/${year}_complete.json`, 'utf-8'));
    let yearFixed = 0;

    for (const q of data) {
      const parts = q.id.split('-');
      const paper = parts[1]; // e.g. '2301'
      const qnum = q.questionNumber;

      if (official[year] && official[year][paper] && official[year][paper][qnum]) {
        const correct = official[year][paper][qnum];
        totalChecked++;

        if (q.answer !== correct) {
          // Fix in Firestore
          try {
            await db.collection('questions').doc(q.id).set({ answer: correct }, { merge: true });
            console.log(`FIXED ${q.id}: ${q.answer} -> ${correct}`);
            yearFixed++;
            totalFixed++;
          } catch (e) {
            console.log(`MISSING ${q.id}`);
            totalMissing++;
          }
        }
      }
    }
    console.log(`\n--- ${year}年: Fixed ${yearFixed} answers ---\n`);
  }

  console.log(`\n=== TOTAL: Fixed ${totalFixed}/${totalChecked} answers (${totalMissing} missing) ===`);
  process.exit();
}
run();
