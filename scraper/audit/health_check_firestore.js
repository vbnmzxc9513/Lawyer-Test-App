...
// Expected question counts per paper
const expectedCounts = {
  111: {'2301':75, '1301':75, '3301':80, '4301':70},
  112: {'2301':75, '1301':75, '3301':80, '4301':70},
  113: {'2301':75, '1301':75, '3301':80, '4301':70},
  114: {'301':75, '302':75, '303':80, '304':70}
};
...
async function run() {
  ...
  log('╔══════════════════════════════════════════════════════╗');
  log('║      Firestore 題庫全面健康檢查報告                  ║');
  log('╚══════════════════════════════════════════════════════╝\n');
  
  let grandTotalDocs = 0;
  let grandAnswerWrong = 0;
  let grandMissingText = 0;
  let grandMissingQnums = 0;
  let grandHasExplanation = 0;
  let grandExplanationMismatch = 0;
  ...
  for (const year of [111, 112, 113, 114]) {
    ...
    log(`  Firestore 總文件數: ${snap.size}`);
    grandTotalDocs += snap.size;
    ...
    for (const doc of snap.docs) {
      ...
      byPaper[paper].push({ id: doc.id, ...data });
    }
    
    const expected = expectedCounts[year];
    const expectedTotal = Object.values(expected).reduce((a,b) => a+b, 0);
    
    if (snap.size !== expectedTotal) {
      log(`  ⚠️ 預期 ${expectedTotal} 題，實際 ${snap.size} 題！`);
    } else {
      log(`  ✅ 題數正確: ${snap.size}/${expectedTotal}`);
    }
    
    for (const [paper, expCount] of Object.entries(expected)) {
      const questions = byPaper[paper] || [];
      log(`\n  --- 試卷 ${paper} (預期 ${expCount} 題) ---`);
      
      if (questions.length !== expCount) {
        log(`  ⚠️ 實際 ${questions.length} 題 (差 ${questions.length - expCount})`);
      } else {
        log(`  ✅ 題數: ${questions.length}/${expCount}`);
      }
      
      // Check question numbers
      const qnums = questions.map(q => q.questionNumber).sort((a,b) => a-b);
      const expectedNums = Array.from({length: expCount}, (_, i) => i + 1);
      const missingNums = expectedNums.filter(n => !qnums.includes(n));
      const extraNums = qnums.filter(n => !expectedNums.includes(n));
      
      if (missingNums.length > 0) {
        log(`  ❌ 缺少題號: ${missingNums.join(', ')}`);
        grandMissingQnums += missingNums.length;
      }
      if (extraNums.length > 0) {
        log(`  ⚠️ 多餘題號: ${extraNums.join(', ')}`);
      }
      ...
      for (const q of questions) {
        ...
        if (!['A','B','C','D'].includes(q.answer)) {
          log(`  ❌ 無效答案 ${q.id}: "${q.answer}"`);
        }
        
        // Check answer correctness
        const officialPaper = official[year]?.[paper];
        if (officialPaper && officialPaper[q.questionNumber]) {
          if (q.answer !== officialPaper[q.questionNumber]) {
            answerWrong++;
            grandAnswerWrong++;
            log(`  ❌ 答案錯誤 ${q.id}: DB=${q.answer}, 官方=${officialPaper[q.questionNumber]}`);
          }
        }
        
        // Check explanation
        if (q.explanation && typeof q.explanation === 'object' && q.explanation.coreConcept) {
          hasExplanation++;
          grandHasExplanation++;
          
          // Check if optionAnalysis references the correct answer
          const optAnalysis = q.explanation.optionAnalysis;
          if (optAnalysis) {
            const correctAns = q.answer;
            const correctAnalysis = optAnalysis[correctAns] || '';
            if (correctAnalysis && !correctAnalysis.includes('正確') && !correctAnalysis.includes('本題答案')) {
              explanationMismatch++;
              grandExplanationMismatch++;
            }
          }
        }
      }
      ...
      if (answerWrong > 0) {
        log(`  ❌ 答案錯誤: ${answerWrong} 題`);
      } else {
        log(`  ✅ 答案全部正確`);
      }
      
      if (missingText > 0) {
        log(`  ⚠️ 題目文字缺失: ${missingText} 題`);
      } else {
        log(`  ✅ 題目文字完整`);
      }
      
      log(`  📝 有詳解: ${hasExplanation}/${questions.length} 題`);
      if (explanationMismatch > 0) {
        log(`  ⚠️ 詳解答案標記疑似不一致: ${explanationMismatch} 題`);
      }
    }
  }
  ...
  log(`  📊 總結報告`);
  log(`${'═'.repeat(60)}`);
  log(`  Firestore 總題數: ${grandTotalDocs}`);
  log(`  答案仍錯誤: ${grandAnswerWrong}`);
  log(`  題目文字缺失: ${grandMissingText}`);
  log(`  題號缺漏: ${grandMissingQnums}`);
  log(`  已有詳解: ${grandHasExplanation}`);
  log(`  詳解答案疑似不一致: ${grandExplanationMismatch}`);
  log(`${'═'.repeat(60)}`);
  
  if (grandAnswerWrong === 0 && grandMissingText === 0 && grandMissingQnums === 0) {
    log(`\n  🎉 基礎資料完整性: ALL PASS`);
  } else {
    log(`\n  🚨 有問題需要處理！`);
  }
  ...
  const fs = require('fs');
  fs.writeFileSync('health_check_report.txt', report.join('\n'), 'utf-8');
  log(`\n報告已寫入 health_check_report.txt`);
